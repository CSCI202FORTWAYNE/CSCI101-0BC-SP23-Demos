...
int main()
{
    ...
    std::cin.ignore();
    startGame(playerList, theDeck, DECK_SIZE, board, BOARD_SIZE);
}
...
void setupPlayers(playerListType &pl)
{
    ...
    for (int i = 0; i < pl.numPlayers; i++)
    {
        ...
        for (int j = 0; j < i; j++)
        {
            ...
            {
                std::cout << pl.playerList[j].name << " has alread picked that color. Please pick again." << std::endl;
                pl.playerList[i].color = inputColor();
                j = -1;
            }
        }
...
colorType inputColor()
{
    ...
    std::cin >> num;
    std::cout << std::endl;
    while (!std::cin || num - 1 < RED || num - 1 > ORANGE)
    {
        if (!std::cin)
        {
            resetStream();
...
void startGame(playerListType &pl, deckType &d, int deckSize, colorType b[], int boardSize)
{
    bool winner = false;
    while (!winner)
    {
        for (int i = 0; i < pl.numPlayers; i++)
        {
            takeTurn(pl.playerList[i], d, deckSize, b, boardSize, winner);
        }
